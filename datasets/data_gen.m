% generate systhetic datasets with varying ep
otl_set = [0.02,0.04,0.06,0.08,0.10];
ep = [0.2,0.4,0.6,0.8,1,2,3,4,5,6];
len = size(ep,2);
for i = 1:len
    d = 100;
    n_size = 100000;
    z = round(otl_set(1)*n_size);
    create_data(n_size,d,8,otl_set(1), ep(i));
end
...
eta = eta(t);
k_standard=k;
filename = ['./datasets_gen/', num2str(ep*10), 'data.mat'];
