...
saveFile = './alg1_k_results/alg1_k.mat';
folderPath = './alg1_k_results';
if ~isfolder(folderPath)
mkdir(folderPath);
end
% looping for k(different datasets)
for i = 1:k_num %enumerate synthetic datasets based on k
    writeFile = ['./alg1_k_results/alg1_k',num2str(k_set(i)),'.txt'];
    ...
    tabulate(X.target);
    fprintf('----KC_alg1_k=%d-dataset -------\n',k_set(i));
    
    % looping for different k_prime
    for beta = 1:ratio_num
        fprintf('----KC_alg1_k %d-dataset %f-k beta=%f-------\n',i,k,beta);
        [n_data,~] = size(data);
