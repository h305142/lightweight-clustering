function []=NKmeans_size()
%---------configuration-----
addpath('./../../../clustering_code/')
addpath('./../../../clustering_code/baseline/kmedian/NKmeans');
size_set = [3,4,5];
...
saveFile='./results/NKmedian_size.mat';
folderPath = './results';
...
for i=1:size_num%enumerate synthetic datasets based on k
    writeFile=['./results/NKmeans_size',num2str(size_set(i)),'.txt'];
    ...
    for j = 1:RPT
        tic;
        %
        [centers,cost(j)] =  NKmeans(X.data,X.k,X.z);
        %         
        timer(j)=toc;
