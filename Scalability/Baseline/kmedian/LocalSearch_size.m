function []=LocalSearch_size()
%---------configuration-----
addpath('./../../../clustering_code/')
addpath('./../../../baseline/kmedian/LocalSearch');
size_set = [3,4,5];
...
saveFile='./results/LocalSearch.mat';
folderPath = './results';
...
for i=1:size_num%enumerate synthetic datasets based on k
    writeFile=['./results/LocalSearch_size',num2str(size_set(i)),'.txt'];
    ...
    data= X.data; k = X.k; z=X.z; ep_lso=0.1;
    tabulate(X.target);
    ...
    for j = 1:RPT
        %
        [centers,timer(j)] = LocalSearch(data,k,z);
        %
