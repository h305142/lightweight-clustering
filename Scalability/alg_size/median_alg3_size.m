...
saveFile='./median_alg3_size_results/alg3_size.mat';
folderPath = './median_alg3_size_results';
if ~isfolder(folderPath)
mkdir(folderPath);
end
for i=1:size_num
    writeFile=['./median_alg3_size_results/alg3_size',num2str(size_set(i)),'.txt'];
    ...
    eta_arr = X.eta; delta_arr = X.delta;         %<1-sqrt(e2/e1)
...
%%    
    for j = 1:RPT
        [centers,sampNum,k_prime_,timer(j)]=algorithm3(data,k,X.z,...
            'sampNum',S_num,'k_prime',k_prime,'Times',Times,'ValdnRatio',ValdnRatio);
        [cost(j), pred_label, pred_otlIdx] = Sum_dist(centers,data,X.z);
