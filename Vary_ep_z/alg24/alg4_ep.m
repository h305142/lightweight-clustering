...
saveFile = './alg4_ep_results/alg4_ep.mat';
folderPath = './alg4_ep_results';
if ~isfolder(folderPath)
mkdir(folderPath);
end
% looping for ep(e1/e2)(different datasets)
for i=1:ep_len %enumerate synthetic datasets based on ep
    writeFile=['./alg4_ep_results/alg4_ep',num2str(ep(i)*10),'.txt'];
    ...
    for zIdx = 1:z_len
        ...
        for j = 1:times
            ...
            [centers,sampNum,z_prime,timer(j)] = algorithm4(data,k,X.z,...
                'sampNum',S_num,'z_prime',z_prime,'Times',5,'ValdnRatio',0.2);
