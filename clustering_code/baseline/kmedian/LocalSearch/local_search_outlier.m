...
% if false 
% centers_weight=weight; 
% centers_weight(:,2)=1:num;
% sortrows(centers_weight,'descend'); 
% centers_index=centers_weight(1:k,2);
% centers=X(centers_index,:); 
% end

%use a function X_outliers_index=outliers
%X_outliers_index=outliers(X,centers,[],z);
%
X_outliers_index=[];
alpha=Inf;
