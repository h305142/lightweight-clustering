function [centers_best,cost_z] = NKmeans1(X,k,z)
z_org=z;
[Y,z,MinDist,MaxDist,p,weight] = sampleCoreset1(X,k,z);
[Y_row,Y_col] = size(Y);
...
for rpt=1:1
    ...
    while l <= ubound
        ...
        [centers] = kmeans_mm_weight(Lloydpointsset,weight(inliersIdx,:),k,z);
        
        [cost_cur,~,~] = Sum_sqdist(centers,X,z_org);
        if cost_cur < cost_z
            cost_z = cost_cur;
