...
saveFile = './median_alg3_results/alg3.mat';
folderPath = './median_alg3_results';
if ~isfolder(folderPath)
mkdir(folderPath);
end
% looping for k(different datasets)
for i = 1:ep_len %enumerate synthetic datasets based on k
    writeFile = ['./median_alg3_results/alg3_ep',num2str(10*ep(i)),'.txt'];
    ...
    for beta = 1:ratio_num
        ...
        for j = 1:times
            [centers,sampNum,k_prime,timer(j)] = algorithm3(data,k,X.z,...
                'sampNum',S_num,'k_prime',k_prime,'Times',3,'ValdnRatio',0.02);
