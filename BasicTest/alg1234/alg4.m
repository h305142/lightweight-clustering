...
saveFile='./alg4_results/alg4.mat';
folderPath = './alg4_results';
if ~isfolder(folderPath)
mkdir(folderPath);
end
% looping for k(different datasets)
for i=1:ep_len %enumerate synthetic datasets based on k
    writeFile = ['./alg4_results/alg4_ep',num2str(10*ep(i)),'.txt'];
    ...
    for j = 1:times
        [centers,sampNum,z_prime,timer(j)] = algorithm4(data,k,X.z,...
            'sampNum',S_num,'z_prime',z_prime,'Times',10,'ValdnRatio',0.02);
